...
const Folder = ({ folder }: { folder: FolderType }) => {
  ...
  function setpageNavigation() {
    setPageNavigation((prevPageNavigation) => {
      if (prevPageNavigation !== null) {
        return [...prevPageNavigation, { path: folder.name, id: folder._id }];
      } else {
        return [{ path: folder.name, id: folder._id }];
      }
    });
  }
  ...
  return (
    <div
      onClick={() => {
        navigate(`/${folder.name}/${folder._id}`);
        setpageNavigation();
      }}
